...
## (제안 솔루션)
  ...
  2. MASTA사의 ABS_TEST 결과 파일로부터, 이상탐지 결과와 비교하기 위한 합격여부 변수를 정의한다.  
    ...
    * '전륜/후륜/사이드' 합격여부는 1차 측정시 합격이면 '합격'으로,  
                                  1차 측정시 불합격했을 때 2차 측정시 합격이면 합격으로 처리한다.(2차 측정시에도 불합격이면 불합격)
    
  3. 변화점 탐지 알고리즘 중 Segment-Neighborhood 방법을 적용한다. 이때, MASTA EV에서 제공한 ABS_TEST 결과 파일로부터 '측정횟수' 만큼의 브레이크 구간을 찾는다. 
...
- **현 이슈를 해결하기 위해, 2월 8일부로 이전차량의 종료시간과 현재차량의 시작시간이 최소 1분의 대기시간을 두고 측정 및 기록될 수 있도록 협조를 구해놓은 상황이며, 2월 말 까지 해당 이슈가 해결된 데이터 100건이 수집된 후 성능평가 결과를 업데이트 할 예정이다.**
<br />
