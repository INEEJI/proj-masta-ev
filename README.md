...
- 현재, MASTA에서는 차량별 테스트 시작시간, 종료시간을 '분' 단위로 측정하고 있다. 하지만 아래와 같은 경우에는 종료시간과 시작시간이 겹쳐 속도센서 DB로부터 데이터를 추출하는 과정에서 중첩되는 영역이 생긴다. 2월 8일부로, 이전차량의 종료시간과 현재차량의 시작시간이 최소 1분의 대기시간을 두고 측정 및 기록될 수 있도록 협조를 구해놓은 상황이며 해당 이슈가 해결된 2월 데이터 100건에 대하여 이를 표준데이터로 사용하여 성능 비교를 진행할 것이다.
